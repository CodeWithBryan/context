...
fn extracts_css_class_and_id_selectors() {
    ...
    assert!(names.iter().any(|n| n == "#hero"), "missing #hero, got: {names:?}");
...
fn extracts_html_tag_id_and_class_symbols() {
    ...
    assert!(names.iter().any(|n| n == "div"), "missing div, got: {names:?}");
    assert!(names.iter().any(|n| n == "span"), "missing span, got: {names:?}");
    assert!(names.iter().any(|n| n == "#app"), "missing #app, got: {names:?}");
    assert!(names.iter().any(|n| n == ".root"), "missing .root class, got: {names:?}");
    assert!(names.iter().any(|n| n == ".layout"), "missing .layout class, got: {names:?}");
}
