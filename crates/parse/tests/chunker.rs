...
fn chunks_tsx_component() {
    ...
    let chunks = Chunker::new()
        .chunk("tests/fixtures/sample.tsx", src.as_bytes())
        .unwrap();
    let names: Vec<_> = chunks.iter().filter_map(|c| c.name.clone()).collect();
    assert!(
        names.contains(&"Button".to_string()),
        "missing Button, got: {names:?}"
    );
}
...
fn chunks_css_selectors() {
    ...
    let chunks = Chunker::new()
        .chunk("tests/fixtures/sample.css", src.as_bytes())
        .unwrap();
...
fn chunks_html_elements() {
    ...
    let chunks = Chunker::new()
        .chunk("tests/fixtures/sample.html", src.as_bytes())
        .unwrap();
    let tags: Vec<_> = chunks.iter().filter_map(|c| c.name.clone()).collect();
    assert!(
        tags.iter().any(|t| t == "div"),
        "missing div element, got: {tags:?}"
    );
}
...
fn chunks_json_as_document() {
    ...
    let chunks = Chunker::new()
        .chunk("tests/fixtures/sample.json", src.as_bytes())
        .unwrap();
...
fn chunker_rejects_unknown_extension() {
    let err = Chunker::new()
        .chunk("sample.py", b"def foo(): pass")
        .unwrap_err();
